...
$$\mathbb{E}[\frac{Z(h)^k}{Z_0^k}] = \exp\left(-V\alpha \int_0^1 dy y [\sqrt{y^2 + k(\frac{\sigma}{(2\pi^2 \alpha)^{1/3}})^2} - y]\ln[2\sinh(\frac{T_D}{2T}y)] + V\alpha \int_0^1 dy\,y^{2}\ln\bigg[2\sinh\bigg(\frac{T_D}{2T}y\bigg)\bigg] \right)$$
